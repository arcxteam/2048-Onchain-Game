...
import { useNFTClaim } from "@/web3/nftClaim";
...
const Overlay: React.FC = () => {
  ...
  if (victory) {
    return (
      <div className="z-999 absolute bottom-0 left-0 right-0 top-0 flex flex-col justify-center bg-[#a3d449] bg-opacity-80 text-center align-middle">
        <h1 className="text-2xl font-bold">You win!</h1>
        <div className="flex justify-center gap-2">
          <button onClick={dismiss}>Keep going</button>
          <button className="rounded-md bg-blue-300 px-3 py-2" onClick={claim}>
            Claim NFT
          </button>
        </div>
      </div>
  ...
  if (defeat) {
    return (
      <div className="z-999 absolute bottom-0 left-0 right-0 top-0 flex flex-col justify-center bg-[#a3d449] bg-opacity-50 text-center align-middle">
